<?php 
	...
	CONCAT(zc.nombre,' - ',zb.nombre) AS ciudad_ocurrencia,
	...
	WHERE  e.id_tipo=5 AND p.id_tipo=21 AND q.id_tipo=12 AND r.id_tipo=17 AND s.id_tipo=5  AND c.tipo_persona=1  AND a.id='".$id_caso."'";
	...
	LEFT JOIN definicion_tipos d ON d.id=a.regimen
	LEFT JOIN definicion_tipos e ON e.id=a.estado
	...
	WHERE 
	a.tipo_persona=2 AND c.id_tipo=5 AND d.id_tipo=18 AND e.id_tipo=19 AND a.id_investigacion='".$id_caso."'";
	mysqli_next_result($con);
...
//FIN CIUDAD Y FECHA
	...
	if ($cont3==1)
	{
		while ($resVariosLesionados=mysqli_fetch_assoc($verificarVariosLesionados))
		{
			...
			if ($resVariosLesionados["edad"]>18 and $resInformacionCaso["sexo"]=="1")
			{
				$textrun->addText(' al señor ',$estiloNoBold);
			}
			else if ($resVariosLesionados["edad"]>18 and $resInformacionCaso["sexo"]=="2")
			{
				$textrun->addText(' a la señora ',$estiloNoBold);
			}
			else if ($resVariosLesionados["edad"]<18 and $resInformacionCaso["sexo"]=="2")
			...
			else if ($resVariosLesionados["edad"]<18 and $resInformacionCaso["sexo"]=="1")
			{
				$textrun->addText(' al joven ',$estiloNoBold);
			}
...
//TERCERA PAGINA
	...
	if ($resInformacionCaso["furips"]!="N")
	{
		...
		mysqli_next_result($con);
		$consultarObservacionesNuevas=$consultarObservacionesInforme." AND id_seccion IN (1,2)";
	...
	if ($resInformacionCaso["diligencia_formato_declaracion"]==1)
	{
		...
		$textrun = $section->createTextRun(array('align'=>'justify','spaceBefore'=>0,'spaceAfter'=>0));  
		$textrun->addText('numero:'.$cantidadInfoDiligencia.'..Quien diligencia el Formato de Declaración de Siniestros es ',$estiloNoBold);
		if ($resInfoDiligencia["edad"]>18 and $resInfoDiligencia["sexo"]=="1")
		{
			$textrun->addText('el señor ',$estiloNoBold);
		}
		else if ($resInfoDiligencia["edad"]>18 and $resInfoDiligencia["sexo"]=="2")
		{
		...
		else if ($resInfoDiligencia["edad"]<18 and $resInfoDiligencia["sexo"]=="1")
		{
    ...
    //HACER TABLA VARIOS LESIONADOS
    	...
    	mysqli_next_result($con);
		$verificarVariosLesionados=mysqli_query($con,$consultarLesionados);	
    	while ($resTablasVariosLesionados=mysqli_fetch_array($con,$verificarVariosLesionados))
		{
		   	$table->addRow(2000);
		 	...
		 	$table->addCell(2000)->addText($resTablasVariosLesionados["tipo_identificacion"].' '.$resTablasVariosLesionados["identificacion"],$estiloNoBold,array('align'=> 'center','spaceBefore'=>0,'spaceAfter'=>0));
		...
		mysqli_next_result($con);
		$verificarVariosLesionados=mysqli_query($con,$consultarLesionados);	
		while ($lesionesOtrosLesionados=mysqli_fetch_assoc($verificarVariosLesionados))
		{
			$a++;
			if($a==1)
			{
			...
			else
			{
				...
				$textrun->addText($lesionesOtrosLesionados["nombre_completo"],$estiloBold); 
				$textrun->addText(" sufrió ".$lesionesOtrosLesionados["lesiones"],$estiloNoBold);  
			}
		} 
		...
		while ($tratamientosOtrosLesionados=mysqli_fetch_assoc($verificarVariosLesionados))
		{
			...
			if($a==1)
			{
				...
				$textrun->addText($tratamientosOtrosLesionados["nombre_completo"],$estiloBold); 
	...
	else
	{
		...
		{
  			$section->addText($cantidadTestigos.".En el lugar de los hechos no se encontraron testigos, a pesar de la labor de campo realizada.",array('bold'=>false,'size'=>11,'name'=>'Arial'),array('align'=> 'justify','spaceBefore'=>0,'spaceAfter'=>0));
		}
...
//$id_caso;
	...
	$textrun = $section->createTextRun(array('align'=>'justify','spaceBefore'=>0,'spaceAfter'=>0));
	$textrun->addText("El Tomador", $estiloNoBold);	
	
	
	$textrun->addText(" de la Póliza SOAT es ", $estiloNoBold);
	$textrun->addText($resInformacionCaso["nombre_tomador"],$estiloBold);
	$textrun->addText(" identificado con ".$resInformacionCaso["tipo_identificacion_tomador"]." ",$estiloNoBold);
	$textrun->addText($resInformacionCaso["no_identificacion_ltomador"],$estiloBold);
	if ($resInformacionCaso["direccion"]!="N")
	{
		$textrun->addText(", ".$resInformacionCaso["direccion"],$estiloNoBold);	
	}

	if ($resInformacionCaso["telefono"]!="N")
	{
		$textrun->addText(", ".$resInformacionCaso["telefono"],$estiloNoBold);	
	}
	...
	$section->addTextBreak(1,null,array('align'=> 'left','spaceBefore'=>0,'spaceAfter'=>0,'lineHeight'=>1));   
   	...
   	$section->addTextBreak(1,null,array('align'=> 'left','spaceBefore'=>0,'spaceAfter'=>0,'lineHeight'=>1));  
	$textrun = $section->createTextRun(array('align'=>'justify','spaceBefore'=>0,'spaceAfter'=>0));  
	if ($resInformacionCaso["seguridad_social"]<>"3")
	{
		if ($resInformacionCaso["seguridad_social"]=="4")
		{
			if ($resInformacionCaso["sexo"]=="1" && $resInformacionCaso["edad"]>18)
			{
				$textrun->addText('El señor ',$estiloNoBold);
			}
			else if ($resInformacionCaso["sexo"]=="1" && $resInformacionCaso["edad"]<18)
			{
				$textrun->addText('El joven ',$estiloNoBold);
			} 
			else if ($resInformacionCaso["sexo"]=="2" && $resInformacionCaso["edad"]>18)
			{
				$textrun->addText('La señora ',$estiloNoBold);
			}
			else if ($resInformacionCaso["sexo"]=="2" && $resInformacionCaso["edad"]<18)
		...
		else
		{
			...
			if ($resInformacionCaso["seguridad_social"]=="1")
			{
			...
			else if ($resInformacionCaso["seguridad_social"]=="2")
			{
	...
	else
	{
		...
		$textrun->addText($resInformacionCaso["nombre_lesionado"].", ",$estiloBold);  
		$textrun->addText($resCausalConsulta["descripcion"],$estiloNoBold);
	...
	if ($cantidadImagenesSeguridadSocial==1)
	{
		unset($arrayImagenes);
		$cont=0;
  	...
  	if (mysqli_num_rows($verificarVariosLesionados)>0)
	{
		...
		while ($resVariosLesionados=mysqli_fetch_assoc($verificarVariosLesionados))
		{
			...
			if ($resInformacionCaso["seguridad_social"]<>"3")
			{
				if ($resVariosLesionados["seguridad_social"]=="4")
				{
					if ($resVariosLesionados["sexo"]=="1" && $resVariosLesionados["edad"]>18)
					{
						$textrun->addText('El señor ',$estiloNoBold);
					}
					else if ($resVariosLesionados["sexo"]=="1" && $resVariosLesionados["edad"]<18)
					{
						$textrun->addText('El joven ',$estiloNoBold);
					} 
					else if ($resVariosLesionados["sexo"]=="2" && $resVariosLesionados["edad"]>18)
					{
						$textrun->addText('La señora ',$estiloNoBold);
					}
					else if ($resVariosLesionados["sexo"]=="2" && $resVariosLesionados["edad"]<18)
				...
				else
				{
					...
					if ($resVariosLesionados["seguridad_social"]=="1")
					{
						$textrun->addText("reporta afiliación al sistema General de Seguridad Social en Salud con ",$estiloNoBold);
					...
					else if ($resVariosLesionados["seguridad_social"]=="2")
					{
						$textrun->addText("no reporta afiliación al sistema General de Seguridad Social en Salud, según ficha técnica Anexa:",$estiloNoBold);
					}
				}
...
//$consultarObservacionesFURIPSnew15=$consultarObservacionesFURIPS." AND id_seccion=13";
	...
	$section->addImage( '../firma.png',array('width' => 100,'height' => 100,'wrappingStyle' => 'behind','align'=>'left','marginLeft'=>0));
	$section->addText("JOSÉ M QUIJANO RODRIGUEZ",$estiloBold,array('align'=>'left','spaceBefore'=>0,'spaceAfter'=>0,'lineHeight'=>1.0));
	$section->addText("GERENTE",$estiloBold,array('align'=>'left','spaceBefore'=>0,'spaceAfter'=>0));
